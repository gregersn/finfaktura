...
class f60Feil(Exception):
    pass


class f60FeilKID(Exception):
    pass
...
def setlocale():
    ...
    for x in (
            'nb_NO.utf8',
            'nn_NO.utf8',  # git bash på Windows
            'nb_NO.UTF-8',
            'nn_NO.UTF-8',  # Ubuntu/Debian
            'nb_NO.UTF8',
            'nn_NO.UTF8',  # ?
...
class F60:
    ...
    def settFirmainfo(self, info: Dict[str, Any]):
    ...
    def settDatoformat(self, format: str):
    ...
    def skrivUt(self, program: str = PDFUTSKRIFT):
    ...
    def lagKontrollsifferMod10(self, tallrekke: str):
    ...
    def settBrukerbegrensning(self, passord: str = '', utskrift: int = 1, endringer: int = 0, kopieringer: int = 0, kommentarer: int = 1):
      ...
      """
        try:
            ...
            return False
        e = StandardEncryption(userPassword=passord,
                               canPrint=utskrift,
                               canModify=endringer,
                               canCopy=kopieringer,
                               canAnnotate=kommentarer,
                               strength=128)
        return self.canvas.setEncrypt(e)

    # ==================== INTERNE FUNKSJONER ================ #

    def sjekkFilnavn(self, filnavn: str):
    ...
    def paragraf(self, t: str, par_bredde: int = 80):
    ...
    def kutt(self, t: str, lengde: int = 200):
    ...
    def lagKlammer(self, punktX: int, punktY: int, deltaX: int, deltaY: int, tekst: Optional[str] = None):
    ...
    def fyll(self):  #, firma, faktura):
        ...
        # pdf-metadata
        self.canvas.setSubject("Faktura #%s til kunde #%s" % (self.faktura['nr'], self.kunde['nr']))
        self.canvas.setTitle("Elektronisk faktura fra %s, utstedt den %s" % (self.firma['firmanavn'], self.faktura['utstedt']))
...
Side: %i av %i
        ...
        for i, _mva in enumerate(mvagrunnlag.keys()):
            ...
            self.canvas.drawString(mvaX, mvaY - (i * 3 * mm),
                                   "%.1f%% av %s = %s" % (_mva, self._kr(linjesum), self._kr(linjesum * _mva / 100)))
...
if __name__ == '__main__':
    ...
    faktura.settFakturainfo(0o3,
                            1145542709,
                            1146546709,
                            "Et forsøk på en faktura",
                            "Takk for handelen, kom gjerne igjen. Merk at det regners 5 % rente ved for sen betaling. ",
                            kid=True,
                            levertEpoch=1145142709)
    faktura.settFirmainfo({
        'firmanavn': 'Test Firma Ein',
        'kontaktperson': 'Rattatta Hansen',
